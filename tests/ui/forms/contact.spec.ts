...
test('Successfully submit form', async ({ pageManager }) => {
    const randomFullName = faker.person.fullName()
    const randomEmail = `${randomFullName.replace(' ', '')}${faker.number.int({max: 1000})}@test.com`
    const randomText = faker.lorem.lines(3)

    await pageManager.onContactPage().fillForm(randomFullName, randomEmail, randomText)
    await pageManager.onContactPage().submitForm()
})
